...
PORT_SCAN_WINDOW = 10          # seconds to observe ports
PORT_SCAN_PORT_THRESHOLD = 12  # distinct ports in the window to trigger an alert

# Keywords to flag in HTTP User-Agent (lowercase)
SUSPICIOUS_UA_KEYWORDS = ["sqlmap", "nikto", "acunetix", "curl", "wget", "fuzzer", "scanner"]

# Optional webhook (set via CLI)
WEBHOOK: str | None = None

# runtime state
syn_history = defaultdict(lambda: deque())  # src_ip -> deque of (timestamp, dst_port)
...
=======
    ...
    if IP in pkt and TCP in pkt:
        ...
        # SYN detection (TCP flag 0x02)
        if int(tcp.flags) & 0x02:
            check_port_scan(ts, src_ip, dst_port)

        # If payload present, check very simply for HTTP User-Agent (plaintext HTTP only)
        if Raw in pkt:
            check_http_user_agent(bytes(pkt[Raw].load), src_ip, dst_ip, dst_port)


def parse_args():
    p = argparse.ArgumentParser(description="Tiny IDS probe - sniff packets and emit simple alerts")
    p.add_argument("-i", "--iface", default=None, help="Interface to sniff (optional). On Windows use NPF name if needed.")
    p.add_argument("--webhook", default=None, help="Optional: webhook URL to POST alerts")
